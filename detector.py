...
class SecurityDetector:
    ...
    def analyze_single(self, log_entry: Dict) -> List[Dict]:
        """
        Analyze a single log entry in real-time.
        Efficient for streaming mode - processes one entry at a time.
        
        Args:
            log_entry: Single parsed log entry
        ...
        # Process the log entry through all rules
        for rule in self.rules:
            event = rule.evaluate(log_entry)
            if event:
                events.append(event)
        
        # Note: We don't call finalize() here for aggregation rules
        # as they need to accumulate data. Finalize should be called
        # periodically or on shutdown for those rules.
        
        return events
    
    def finalize_aggregation_rules(self) -> List[Dict]:
        """
        Finalize aggregation-based rules and return accumulated events.
        Should be called periodically or on shutdown.
        
        Returns:
            List of events from aggregation rules
        """
        events = []
        
        for rule in self.rules:
            if hasattr(rule, 'finalize'):
                rule_events = rule.finalize()
                if rule_events:
    ...
    def get_session_statistics(self) -> Dict:
        """
        Get current session statistics from all detected events.
        Useful for real-time monitoring summary.
